...
|---|---|---|---|
| ArcGIS Online `gisdb_gis_regrid_surveys` | Returned survey submissions, period list, history-map survey polygons | Daily upstream publish | **Live query at page load** via `docs/landcare/survey-layer.js` |
...
|---|---|---:|---|---|
| ArcGIS Online survey layer | Returned survey evidence, `period_label` history, daily freshness | AGOL item `a4012693d5d74dd8998610c4d235068d`; refreshed daily by upstream pipeline | **Primary runtime source** for returned surveys in web app | Monitor via AGOL `dataLastEditDate` and upstream `\GIS Automations\REGRID` task |
...
| AGOL publish | `publish_regrid_snapshot.py` overwrites item `a4012693d5d74dd8998610c4d235068d` rather than creating duplicate layers |
...
| Historical monthly parcel map | `docs/landcare/data/all_months.geojson` plus live ArcGIS `gisdb_gis_regrid_surveys` in the web app | Geometry originates from `gis.pgh_parcels` and `gis.epp_parcels_full`; returned survey evidence refreshed from AGOL at page load | `prototype/sql/export_prototype_data_readonly.sql` joins assignment, survey, geometry, and owner tables; `docs/landcare/survey-layer.js` merges live survey layer evidence | None | Postgres export plus live survey layer query in `docs/landcare/monitoring.js` and `docs/landcare/kpi.js` |
...
| Live ArcGIS Regrid survey layer | Refreshed daily by upstream `publish_regrid_snapshot.py` from `gis.regrid_surveys` | Upstream pipeline doc in `URA-Data-Repository`; AGOL item `gisdb_gis_regrid_surveys` | Metadata and map context; historical assurance KPIs still come from Postgres export in this repo |
...
| ArcGIS Regrid survey layer vs Postgres survey periods | AGOL layer is rebuilt daily upstream from `gis.regrid_surveys`; Postgres raw submissions remain the source of truth | `docs/upstream-regrid-survey-pipeline.md` and upstream `docs/regrid-survey-pipeline.md` document the split | Compare AGOL `gisdb_gis_regrid_surveys` max `period` to `gis.regrid_survey_submissions`; monitor `gis.regrid_survey_unmatched_parcels` for geometry-match issues |
