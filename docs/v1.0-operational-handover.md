...
| Browser print visual check | Manual print-preview inspection of the executive map brief | Required before presentation if export is shown live |
| VM scheduler proof | Successful manual and scheduled VM run with status JSON evidence | Required on the target VM after deployment/update |
...
1. Confirm the target VM task has run successfully on the released `master` commit.
2. Open both GitHub Pages surfaces and complete the manual map export print-preview check.
3. Assign an operations owner for the 4:00 AM upstream and 7:00 AM downstream alerts.
4. Use the presentation script to align leadership on denominator rules and contractor follow-up expectations.
