...
- Include one workflow screenshot that shows the `Take Worker Survey` button and opened field task form. The interaction should feel like a Regrid/ArcGIS field worker opening a parcel task, confirming parcel and contractor, recording completion status, adding evidence, and submitting to a shared survey contract.
- Use the local evidence captures in `outputs/proposal-evidence/` for deck styling: `regrid-landcare-network-reference.png`, `ura-ownership-style-reference.png`, and `landcare-proposal-survey-flow.png`. Keep the Regrid screenshot out of public deliverables if it exposes account/session details.
